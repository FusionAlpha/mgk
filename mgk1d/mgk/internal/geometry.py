...
def evaluate_geometry(theta, cfg) -> Struct:
    ...
    if getattr(cfg, "geometryModel", "s-alpha") == "stellarator":
        ...
        return Struct(
            ...
            gyroWaveNumberSquared=k_perpendicular**2,
            kPerpendicularDerivative=cfg.kt * metric_derivative,
            gyroWaveNumberDerivative=cfg.kt * metric_derivative,
    ...
    # Match the MATLAB s-alpha convention.  The pressure-gradient drift is
    # alpha/q^2 multiplied by the inverse field strength, not a constant.
    pressure_drift = cfg.alpha / (cfg.q**2) * inverse_field
    return Struct(
        ...
        metricWaveNumberSquared=metric_k2, kPerpendicularSquared=metric_k2,
        gyroWaveNumberSquared=local_k2,
        kPerpendicularDerivative=cfg.kt * (
            shear * shear_derivative / np.sqrt(1 + shear**2)
        ),
        gyroWaveNumberDerivative=wave_derivative,
        driftCurvature=(np.cos(theta) + shear * np.sin(theta)
                        - pressure_drift),
        driftParallelCorrection=pressure_drift,
...
def _evaluate_miller(theta, cfg):
    ...
    return Struct(
        ...
        logFieldDerivative=log_field,
        kPerpendicularSquared=(cfg.kt * metric)**2,
        gyroWaveNumberSquared=wave**2,
        kPerpendicularDerivative=cfg.kt * metric_derivative,
        gyroWaveNumberDerivative=wave_derivative,

...
def test_geometry_matches_s_alpha_definition():
    ...
    np.testing.assert_allclose(geometry.kPerpendicularSquared,
                               cfg.kt**2 * (1 + shear**2), atol=1e-14)
    np.testing.assert_allclose(geometry.gyroWaveNumberSquared,
                               cfg.kt**2 * (1 + shear**2) * inverse_field**2,
                               atol=1e-14)
    correction = cfg.alpha / cfg.q**2 * inverse_field
    ...
    expected_drift = (
        ...
        - cfg.alpha * inverse_field * perpendicular_speed**2 / (2 * cfg.q**2)
    )

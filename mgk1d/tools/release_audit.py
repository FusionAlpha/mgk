...
REQUIRED_DOCS = (
    "README.md", "CONFIGURATION_REFERENCE.md", "GPU_GUIDE.md",
...
def audit(artifacts: Iterable[Path], allow_license_pending: bool) -> dict:
    ...
    if license_exists:
        _add(checks, "company-approved LICENSE", True, "LICENSE present")
        _add(checks, "PEP 639 license metadata",
             bool(project.get("license") or project.get("license-files")),
             "pyproject contains license metadata")
        _add(checks, "pending legal marker removed",
             not (ROOT / "LEGAL_REVIEW_REQUIRED.md").exists()
             and "LEGAL_REVIEW_REQUIRED.md" not in manifest,
             "remove the pending marker from a formally licensed release")
    elif allow_license_pending:
        _add(checks, "company-approved LICENSE", False,
        ...
        _add(checks, "pending legal marker",
             (ROOT / "LEGAL_REVIEW_REQUIRED.md").is_file()
             and "LEGAL_REVIEW_REQUIRED.md" in manifest,
             "technical candidate is explicitly labelled")
    else:
        _add(checks, "company-approved LICENSE", False,
        ...
        _add(checks, "pending legal marker",
             (ROOT / "LEGAL_REVIEW_REQUIRED.md").is_file(),
             "source records the release blocker", blocking=False)
...
def _markdown(report: dict) -> str:
    ...
    lines.extend([
        ...
        "- Install the company-approved LICENSE/EULA and matching PEP 639 metadata.",
